...
const Item = ({ item, displayItem, displayItemForm }) => {
	...
	console.log("Length: " +item.photos.length)
			...
			<div className='h-full w-full p-6 border border-solid rounded-md grid grid-row-2 '>
				...
				<div className='row-span-1 '>
					{Object.keys(item.photos).map(key => {
						return (
							<div
								className={key? 'opacity-80 scale-110 ease-linear duration-100' : 'opacity-0'}
								key={key}
							>
								{key  && (
									<div className='w-full h-full flex items-center justify-center' >
										<div className='w-full h-full'>
											...
											<img src={`data:image/png;base64,${item.photos[key]}`} alt={"item.description"} className='w-full h-full p-2' />
										</div>
									</div>
								)}
							</div>
